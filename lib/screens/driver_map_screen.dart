...
class _DriverMapScreenState extends State<DriverMapScreen>
      ...
      (event) async {
        try {
          ...
          if (status == 'pending_driver_action') {
            ...
            if ((_activePopupRideId != rideId || !_popupOpen) &&
                !_hasActivePopup) {
              final matchedRide = _matchRideForPopup(
                rideId,
                rideData,
              ...
              if (matchedRide != null) {
                _logReqDebug('popup OPEN rideId=${matchedRide.rideId}');
                unawaited(
                  showRideRequestPopup(
                    matchedRide,
                    rideAlreadyReserved: true,
                  ),
                );
              }
            }
  ...
  Future<bool> _listenForRideRequests({required String reason}) async {
    ...
    Future<void> processSnapshot(
      ...
      required int listenerToken,
        ...
        // Only [_driverActiveRideId] / [_currentRideId] block discovery.
        // [_activeRideListenerRideId] can lag RTDB and must not suppress new requests.
        if (_driverActiveRideId?.trim().isNotEmpty == true ||
            _currentRideId?.trim().isNotEmpty == true) {
          _logDriverReq(
            'skippedReason=active_trip_guard driverActive=$_driverActiveRideId '
            'current=$_currentRideId source=$source',
          );
          _logRideReq(
            'snapshot skipped active-trip guard driverActive=$_driverActiveRideId '
            'current=$_currentRideId source=$source',
          );
          _logDiscoveryChain(
            'snapshot_blocked active_ride_guard current=$_currentRideId '
            'driverActive=$_driverActiveRideId source=$source',
          );
          return;
        }
  ...
  Future<void> showRideRequestPopup(
    ...
    try {
      ...
      if (!rideReserved) {
        final reservedRide = await _reserveRideForPopup(
          ride: popupRide,
        ...
        if (reservedRide == null) {
          _logRideReq(
            'popup blocked reason=reserve_failed rideId=${popupRide.rideId}',
          );
          _clearRidePreview();
          _rearmRideRequestListener(
            'assignment blocked rideId=${popupRide.rideId}',
          );
          return;
        }
        rideReserved = true;
        popupRide = reservedRide;
      }
    ...
    try {
      ...
      final transactionResult = await ref.runTransaction((currentData) {
        ...
        if (!TripStateMachine.isPendingDriverAssignmentState(
            ...
            currentDriverId != _effectiveDriverId) {
          blockedReason = _popupServerSkipReason(rideId, current) ??
              'ride_not_reserved_for_driver';
          return rtdb.Transaction.abort();
        }

        final assignmentExpiresAt = _assignmentExpiryTimestamp(current);
        if (_assignmentHasExpired(current) &&
          ...
          blockedReason = 'assignment_expired';
          return rtdb.Transaction.abort();
        }
      ...
      _suppressedRidePopupIds.add(rideId.trim());
      _foreverSuppressedRidePopupIds.add(rideId.trim());
